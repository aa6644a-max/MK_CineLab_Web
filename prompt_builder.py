...
class PromptBuilder:
    def _get_base_guideline(self):
        ...
        3. 포스팅 레이아웃 구조 (서론-본론-결론):
            ...
            - [서론]: 콘텐츠를 보게 된 계기, 영화의 기본 정보 요약, 포스팅 목적을 밝히세요.
            - [본론]: H2, H3 태그를 활용해 소제목으로 단락을 구분하세요. 내용 흐름에 맞게 아래 제공된 [스틸컷 1] 부터 마지막 스틸컷까지의 HTML 코드를 문단 사이사이에 골고루 흩뿌려서 모두 삽입하세요.
            - [결론]: 전체적인 감상을 갈무리하며 나만의 한줄평과 별점을 직관적으로 제시하세요. 결론부 직전에 반드시 [관람 인증샷] HTML 코드를 삽입하세요.

        4. 멀티미디어 및 이미지 가이드 (🚨 절대 준수):
            - 하단에 제공되는 이미지 HTML 코드들([메인 포스터], [스틸컷 N개], [관람 인증샷])을 1글자도 수정하지 말고 그대로 복사해서 지시된 위치에 넣으세요.
            - 제미나이 임의로 이미지 태그나 박스를 새로 만들어내지 마세요. 오직 제공된 텍스트만 복사&붙여넣기 해야 합니다.
    ...
    def build_preview_prompt(self, details, point, latest_news=""):
        ...
        [제공되는 실제 이미지 HTML 코드 (🚨그대로 복사해서 지시된 위치에 붙여넣으세요)]
        - [메인 포스터]: {poster_html}
{stills_prompt_text}
        - [관람 인증샷]: {ticket_html}
