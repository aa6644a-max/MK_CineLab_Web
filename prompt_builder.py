...
class PromptBuilder(BasePromptBuilder):
    ...
    def _get_base_guideline(self, post_type="review"):
        ...
        if post_type == "preview":
            ...
            - [본론 구성 - MK 레이아웃 4단계 반드시 준수]:
              본론은 아래 4개 소제목 구조를 순서대로 작성하세요. 각 단락 사이에 스틸컷을 고르게 배치하세요.
              ① <h2 style="border-bottom: 2px solid #333; padding-bottom: 5px;"> 연출·시각적 스타일 분석
                 → 감독의 연출 의도, 촬영 기법, 색감·미장센 등을 분석적으로 서술
              ② <h2 style="border-bottom: 2px solid #333; padding-bottom: 5px;"> 배우 연기 & 캐릭터
                 → 주연·조연의 감정 표현, 캐릭터 설득력을 구체적으로 평가
              ③ <h2 style="border-bottom: 2px solid #333; padding-bottom: 5px;"> 주제 & 메시지
                 → 영화가 전달하는 핵심 주제, 사회적·철학적 의미 분석
              ④ <h2 style="border-bottom: 2px solid #333; padding-bottom: 5px;"> MK의 관전 포인트
                 → 개인적 기대감, 이 영화만의 차별점, 추천 대상 명시
              - 각 소제목 아래 2~4문장 단락 구성. 복잡한 요소 설명 시 불렛 포인트(✔ 또는 •)를 1개 이상 필수 삽입."""
