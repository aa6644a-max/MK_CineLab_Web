...
with tab1:
    ...
    # 💡 데이터가 있을 때만 미리보기 실행 (TypeError 방지)
...
with tab4:
    ...
    if not posts:
        ...
        selected_id = st.selectbox("포스팅 선택:", options=list(post_options.keys()), format_func=lambda x: post_options[x])
        if selected_id:
            content = db.get_post_content(selected_id)
            if content:
                components.iframe(srcdoc=content, height=800, scrolling=True)
                if st.button("🗑️ 삭제"):
                    if db.delete_post(selected_id): st.rerun()