...
class LocalNewsPromptBuilder(BasePromptBuilder):
    """
    MK LINK 로컬 소식 포스팅 전용 프롬프트 빌더.
    교육/채용/공모전/지원사업 등 공고문 기반 포스팅에 사용합니다.

    영화·맛집 리뷰와 달리 '정보 전달 + 신청 유도'가 목적이므로
    감상 도입부 없이 직접 선언 구조를 사용합니다.
    """

    def __init__(self):
        super().__init__()

    def _get_common_constraints(self) -> str:
        return ""
    ...
    def _get_local_news_design_system(self, brand_color: str = "#1a2e4a") -> str:
        """
        [🎨 MK LINK 로컬 소식 디자인 시스템]
        네이버 블로그 모바일 최적화 기준.
        - <div> 배경색 대신 <table bgcolor> 사용 (네이버 블로그 렌더링 보장)
        - 다열 테이블 금지 → 단일 컬럼 카드 구조 (모바일 가독성)
        - flex / grid / inline-block 레이아웃 금지
        """
        return f"""
[🎨 MK LINK 디자인 시스템 — 반드시 준수]

━━ 1. 상단 헤더 박스 (모집/행사 제목 선언) ━━
<table width="100%" border="0" cellpadding="22" cellspacing="0" bgcolor="{brand_color}" style="margin-bottom:24px;">
  <tr><td>
...
<table width="100%" border="0" cellpadding="12" cellspacing="0" bgcolor="{brand_color}" style="margin-bottom:0;">
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" style="border:1px solid #dde3ed; border-top:none; margin-bottom:28px; font-size:14px;">
  <tr>
    <td width="30%" bgcolor="#f4f7fb" style="padding:13px 14px; font-weight:bold; color:{brand_color}; border-bottom:1px solid #eef0f5; border-right:1px solid #eef0f5; vertical-align:top;"><b>[항목명]</b></td>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#e8f0fe" style="margin-bottom:2px;">
  <tr><td style="padding:10px 14px;"><b style="color:{brand_color}; font-size:14px;">[N일차 · 월 일(요일)]</b></td></tr>
</table>
...
"""
    ...
    def _get_local_news_constraints(self) -> str:
        return """
[🚫 로컬 소식 포스팅 절대 금지 사항]
...
"""
    ...
    def build_announcement_prompt(
        ...
        pdf_text: str,
        user_context: str,
        brand_color: str = "#1a2e4a",
        reference_posts: str = "",
    ) -> str:
        """
        교육 / 채용 / 공모전 / 지원사업 등 공고문 기반 로컬 소식 포스팅 프롬프트.

        Args:
            pdf_text:        PDF에서 추출한 원문 텍스트
            user_context:    이 소식을 소개하는 목적 및 독자 설명
            brand_color:     섹션 헤더 배경색 (기본 네이비 #1a2e4a)
            reference_posts: MK 문체 레퍼런스 (선택)
        """
        time_context = f"현재 시점은 {self.current_year}년 {self.current_month}월({self.season})입니다."
        design_system = self._get_local_news_design_system(brand_color=brand_color)
        constraints = self._get_local_news_constraints()
        common = self._get_common_constraints()
